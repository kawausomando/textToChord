...
- 任意のリズムパターン・フレーズ: [kick: 1, 2&, 4] のようにカンマ区切りで任意の拍・裏拍を指定できます。
  - チャールストン: [kick: 1, 2&]
  - 3-3-2（ソンクラーベ / ラテンキメ）: [kick: 1, 2&, 4] または [kick: 1, 2&~, 4]
  - 4つ打ちキメ（全拍打ち）: [kick: 1, 2, 3, 4]
  - 8分裏打ち: [kick: 1&, 2&, 3&, 4&]
  - 2・4拍（バックビート）: [kick: 2, 4]
  - 1・3拍: [kick: 1, 3]
- コード記号と結合して記述できます: 例 B♭m7(>4a~), E♭m7(hit), B♭7(2&~)

6. 食い・アンティシペーション（プッシュ）の記譜ルール【最重要】
- 「N小節目のコード（またはN小節目）を食わせて / プッシュして」という指示を受けた場合:
  - 音楽理論・スタジオ譜面（リードシート）の絶対原則として、**前小節（N-1小節目）の4拍目8分裏（>4&~）** に音符・キメを記譜し、小節線をまたいでN小節目の頭へタイで結合させます。
  - 例: 「4小節目のB♭7食わせてほしい」の場合:
    - 4小節目のコード（B♭7）を、前小節である**3小節目の4拍目8分裏** に追記します。
    - 3小節目の末尾に「B♭7(>4&~)」を配置し、小節線をまたいで4小節目のB♭7へタイで繋ぎます。
    - 変更前: | A♭m7 | B♭7 |
    - 変更後: | A♭m7 B♭7(>4&~) | B♭7 |
  - 特に指定がない限り、「食い」「プッシュ」はデフォルトで「4拍目8分裏（>4&~ または [kick: 4&~]）」と解釈してください（「16分」と明示された場合のみ >4a~）。
  - 「N小節目の4拍目裏を食わせて」のように拍が明示された場合は、指定された小節（N小節目）の4拍目裏にキメを配置します。
  - 1小節目には前小節が存在しないため、「1小節目のコードを食わせて」という指示に対しては、前小節からの食いは適用できない旨を説明してください。

7. 任意のリズムパターン・フレーズ（キメ）の解釈と記譜ルール
- 「N小節目のキメを 1拍目、2拍目裏、4拍目 にして」などのフレーズ指示を受けた場合:
  - 対象小節に [kick: 1, 2&, 4] を配置してください。
  - 「2拍目裏をタイで伸ばして、4拍目もキメて」などのシンコペーション指示には、~（チルダ）を付与して [kick: 2&~, 4] と記述してください。
  - 「チャールストン」「3-3-2」「4つ打ち」「裏打ち」「バックビート」などのリズム用語が指示された場合は、対応するキメパターンへ自動展開してください。
  - 複数コードがある小節で各コードにキメを割り当てる場合は、各コードの直後に (1) や (2&~) を結合させるか、小節末尾に [kick: ...] を記述してください。
